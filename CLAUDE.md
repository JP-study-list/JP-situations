...
- [ ] KANJI_READINGS **兩道檢查**（抄 `app.js` 的最長匹配邏輯跑，Node 或 Python 皆可）：
      ...
      **但第 2 道有結構性盲區**：字典讀音與 `kana` 欄位**錯得一模一樣**時，
      反向重建會完全吻合而判為乾淨（2026-08-08 查出 9 筆漏網，見 §8.13）。
      修既有頁時要另跑**第 3 道：跨頁交叉比對同一個詞的讀音**，用正確的頁當 oracle
...
- **假名資料的三類問題要分開看**（2026-08-07 全站盤點的分類，日後沿用）：
  ...
  **另有 B' 類：字典讀音與 `kana` 欄位同錯**——兩道檢查一律判為乾淨，
  只能靠跨頁比對同一詞的讀音抓出來（如 `pharmacy` 的 `入荷=にゅうか`
  對上 `izakaya` 的 `いっか`）。2026-08-08 查出 7 頁共 9 筆仍在線上，
  清單與**對照組**（哪些看起來像錯其實是對的）在 `project-index.md` 的「後續規劃」§二。
